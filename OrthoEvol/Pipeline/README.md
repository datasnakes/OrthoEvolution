...
This module uses Luigi and SunGrid Engine (SGE) to distribute computational
tasks across cluster nodes. Tasks are designed to run on clusters that use
**pbspro or SunGrid Engine**.
...
The `TestPipelineTask` is a simple example that demonstrates how to create
and run a pipeline task:
...
All pipeline tasks inherit from `SGEPipelineTask` and support the following
parameters:
