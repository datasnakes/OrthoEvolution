...
a = mana.ProjMana(repo=repo, user=user, project=project, research=research, research_type=research_type)
# shutil.copy(str(a.index / Path('MAFV3.2.csv')), str(a.project_index))
# Path.mkdir(a.raw_data / Path('blast') / Path('gi_lists'), parents=True)
shutil.copy(str(a.index / Path('get_gi_lists.sh')), str(a.raw_data / Path('blast') / Path('gi_lists')))
